library(tidyverse)
library(progress)
source("Data-Collection-Scripts/Unit-Fix.R")

data_vec <- list.files(path = "Data/Downloaded/Chemical", pattern = "*.csv")
Obsv_list <- list()

df_names <- c("obs_id","MonitoringLocationIdentifier","ActivityStartDate","ActivityStartTime.Time","pH.std units","Specific conductance.uS/cm","Temperature, water.deg C","Alkalinity.ueq/kgw","Organic carbon.umol/l","State")
col_names <- data.frame(matrix(ncol = 10, nrow=0))
    ...
    dat <- dat %>%
      filter(
        (CharacteristicName=="Temperature, water"&ResultMeasure.MeasureUnitCode=="deg C")|
          (CharacteristicName=="Specific conductance"&ResultMeasure.MeasureUnitCode=="uS/cm")|
          (CharacteristicName=="pH"&ResultMeasure.MeasureUnitCode=="std units")|
          (CharacteristicName=="Organic carbon"&ResultMeasure.MeasureUnitCode=="umol/l")|
